...
const accounts = ref<FinanceAccount[]>([
    { id: `acc-${Date.now()}-1`, name: 'Tiền mặt', initialBalance: 0 },
    { id: `acc-${Date.now()}-2`, name: 'Tài khoản Ngân hàng', initialBalance: 0 }
]);
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-base dark:bg-base-dark">
      <div class="max-w-xl w-full flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-8 duration-500">
          ...
          <div class="text-center">
              ...
              <h1 class="text-3xl font-bold text-text dark:text-text-dark mb-2">Chào mừng đến với Synabit Finance</h1>
              <p class="text-gray-500 dark:text-gray-400">Hãy thiết lập các tài khoản và số dư hiện có để bắt đầu hành trình quản lý tài chính của bạn.</p>
          </div>
          
          <div class="bg-surface dark:bg-surface-dark border border-border dark:border-border-dark rounded-3xl p-6 shadow-xl">
              <h2 class="text-lg font-bold text-text dark:text-text-dark mb-4">Khai báo Tài sản hiện tại</h2>
              ...
              <div class="space-y-3 mb-6">
                  <div v-for="(acc, idx) in accounts" :key="acc.id" class="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-border dark:border-border-dark">
                      <div class="flex-1">
                          <input type="text" v-model="acc.name" class="w-full bg-transparent border-none font-medium text-text dark:text-text-dark focus:outline-none focus:ring-0 p-0 mb-1 text-sm" placeholder="Tên tài khoản" />
                          <div class="relative">
                              <input type="text" inputmode="numeric" :value="acc.initialBalance.toLocaleString('vi-VN')" @input="updateBalance(idx, $event)" class="w-full bg-transparent border-none text-xl font-bold text-gray-700 dark:text-gray-300 focus:outline-none focus:ring-0 p-0" placeholder="0" />
                              <span class="absolute left-0 bottom-full text-[10px] uppercase text-gray-400 font-bold tracking-wider">Số dư</span>
                          </div>
                      </div>
              ...
              <div class="flex flex-col gap-2 p-3 bg-blue-50/50 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-900/30 border-dashed mb-6">
                  <input type="text" v-model="newAccountName" class="w-full bg-white dark:bg-gray-800 border border-border dark:border-border-dark rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Thêm tài khoản khác (Vd: Techcombank)" />
                  <div class="flex gap-2">
                      <div class="relative flex-1">
                          <input type="text" inputmode="numeric" :value="newAccountBalance" @input="handleBalanceInput" class="w-full bg-white dark:bg-gray-800 border border-border dark:border-border-dark rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 pr-8" placeholder="Số dư hiện có" />
