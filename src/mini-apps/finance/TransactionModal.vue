...
const save = () => {
    ...
    const tx: Transaction = {
        ...
        category: type.value === 'transfer' ? 'Chuyển khoản' : category.value,
...
const canSave = computed(() => {
    ...
    if (type.value === 'transfer' && accountId.value === toAccountId.value) return false;
    return true;
});
  ...
  <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 dark:bg-black/70 backdrop-blur-sm" @click.self="emit('close')">
    <div class="bg-surface dark:bg-surface-dark border border-border dark:border-border-dark rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-in zoom-in-95 duration-200">
      ...
      <div class="p-5 space-y-4">
        ...
        <!-- Amount -->
        <div>
            <label class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Số tiền</label>
            <div class="relative">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div v-if="type !== 'transfer'">
                <label class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Danh mục</label>
            ...
            <!-- From Account -->
            <div>
                <label class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">{{ type === 'transfer' ? 'Từ tài khoản' : 'Tài khoản' }}</label>
                <select v-model="accountId" class="w-full bg-gray-50 dark:bg-gray-800 border border-border dark:border-border-dark rounded-xl px-3 py-2.5 text-sm text-text dark:text-text-dark focus:outline-none focus:ring-2 focus:ring-blue-500 appearance-none">
            ...
            <div v-if="type === 'transfer'">
                <label class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Đến tài khoản</label>
                <select v-model="toAccountId" class="w-full bg-gray-50 dark:bg-gray-800 border border-border dark:border-border-dark rounded-xl px-3 py-2.5 text-sm text-text dark:text-text-dark focus:outline-none focus:ring-2 focus:ring-blue-500 appearance-none">
                    <option v-for="acc in accounts" :key="acc.id" :value="acc.id">{{ acc.name }}</option>
                </select>
            </div>
        </div>
        ...
        <!-- Date -->
        <div>
            <label class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Thời gian</label>
        ...
        <!-- Note -->
        <div>
            <label class="block text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Ghi chú</label>
            <input type="text" v-model="note" class="w-full bg-gray-50 dark:bg-gray-800 border border-border dark:border-border-dark rounded-xl px-3 py-2.5 text-sm text-text dark:text-text-dark focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Chi tiết giao dịch..." />
        </div>
      ...
      <div class="p-4 border-t border-border dark:border-border-dark flex justify-end gap-3 bg-gray-50/50 dark:bg-gray-800/50">
        ...
        <button @click="save" :disabled="!canSave" class="px-5 py-2 rounded-xl text-sm font-medium bg-blue-500 hover:bg-blue-600 text-white shadow-sm transition-colors disabled:opacity-50">
            Lưu giao dịch
        </button>
      </div>
