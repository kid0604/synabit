...
<template>
  <div class="absolute right-0 top-6 w-44 bg-white dark:bg-[#2c2c2c] shadow-lg rounded border border-gray-200 dark:border-gray-700 z-50 py-1 overflow-hidden">
    ...
    <template v-if="variant !== 'manager'">
      ...
      <button @click.stop="emit('rename', noteId)" class="w-full text-left px-3 py-2 text-xs whitespace-nowrap hover:bg-gray-100 dark:hover:bg-gray-600 flex items-center gap-2">
        <Edit2 class="w-3 h-3" /> {{ $t('note.rename') }}
      </button>
      <button v-if="appLockStore.isEnabled" @click.stop="emit('toggle-lock', noteId)" class="w-full text-left px-3 py-2 text-xs whitespace-nowrap hover:bg-gray-100 dark:hover:bg-gray-600 flex items-center gap-2">
