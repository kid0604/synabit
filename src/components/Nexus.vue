...
const renderMarkdownPreview = (text: string, type: string) => {
    ...
    html = html.replace(/!\[.*?\]\((.*?)\)/g, (match, path) => {
        ...
        return `<img src="${src}" class="max-w-full rounded-xl my-4 border border-gray-200 dark:border-[#333]" loading="lazy" />`;
    });
...
<template>
  <div class="h-full w-full flex relative overflow-hidden bg-[#fdfdfc] dark:bg-[#121212] font-sans">
    ...
    <!-- Main UI -->
    <div class="flex-1 flex flex-col h-full bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-indigo-50/40 to-[#fdfdfc] dark:from-indigo-900/10 dark:to-[#121212] transition-all"
        ...
        <div class="relative w-full max-w-3xl mx-auto pt-16 px-8 pb-8 flex-shrink-0">
            <div class="text-center mb-10 mt-6">
                ...
                <p class="text-[#52525b] dark:text-[#a1a1aa] mt-3 max-w-md mx-auto">Omni-search across all your Notes, Tasks, and QuickCaps in an instant.</p>
            </div>
    ...
    <div class="absolute top-0 right-0 h-full w-[400px] border-l border-gray-200 dark:border-[#2c2c2c] bg-[#fbfbfc] dark:bg-[#1a1a1a] shadow-2xl transition-transform duration-300 ease-out z-30"
         :class="selectedItem ? 'translate-x-0' : 'translate-x-full'">
        
        <div class="flex flex-col h-full" v-if="selectedItem">
            <!-- Header -->
            <div class="h-16 border-b border-gray-200 dark:border-[#2c2c2c] flex items-center justify-between px-6 flex-shrink-0 bg-white/80 dark:bg-[#1e1e1e]/80 backdrop-blur-md">
                <div class="flex items-center gap-3">
                    <div class="p-2 rounded-lg" :class="getTypeColor(selectedItem.item_type)">
                ...
                <button @click="closePreview" class="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-white/10 text-gray-500 transition-colors">
                    <ChevronRight class="w-5 h-5" />
                </button>
            </div>

            <!-- Content Area (Read-only for now) -->
            <div class="flex-1 overflow-y-auto px-8 py-8 bg-white dark:bg-[#1a1a1a]">
                <h2 class="text-3xl font-extrabold text-[#1c1c1e] dark:text-white mb-6 leading-tight tracking-tight">{{ selectedItem.title }}</h2>
                
                <div class="flex flex-wrap gap-2 mb-8" v-if="selectedItem.tags.length">
                    <span v-for="tag in selectedItem.tags" :key="tag" class="text-xs font-medium px-2.5 py-1 rounded bg-gray-100 dark:bg-white/10 text-gray-700 dark:text-gray-300">
                        {{ tag }}
                    </span>
                </div>

                <div class="prose prose-sm dark:prose-invert prose-zinc max-w-none leading-loose">
                    <div class="whitespace-pre-wrap font-sans text-[15px] text-[#333] dark:text-[#ccc] p-0 break-words preview-markdown" v-html="renderMarkdownPreview(selectedItem.content, selectedItem.item_type)"></div>
                </div>
                
                <div class="mt-12 p-4 bg-gray-50 dark:bg-[#222] rounded-xl border border-gray-200 dark:border-[#333]">
                    <div class="text-xs font-medium text-gray-500 flex justify-between items-center">
                        <div class="flex-1 min-w-0">
                            <span class="block opacity-70 mb-1">File Path:</span>
            ...
            <div class="p-4 border-t border-gray-200 dark:border-[#2c2c2c] bg-gray-50 dark:bg-[#1e1e1e] flex-shrink-0 text-center">
                <p class="text-xs font-medium text-gray-400">View Mode Only. Edit in primary tabs.</p>
            </div>
        </div>
    </div>
    
    <!-- Backdrop for mobile/smaller screens -->
    <div v-if="selectedItem" @click="closePreview" class="absolute inset-0 z-20 bg-black/5 dark:bg-black/20 backdrop-blur-[1px] transition-opacity 2xl:hidden"></div>
