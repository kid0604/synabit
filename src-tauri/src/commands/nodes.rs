                ...
                if ext == "md" {
                    if let Ok(content) = std::fs::read_to_string(entry.path()) {
                        if let Ok(parsed) = matter.parse::<serde_yaml::Value>(&content) {
                            ...
                            if let Ok(_) = std::fs::write(entry.path(), file_content) {
...
pub fn copy_asset_to_vault(vault_path: String, source_path: String) -> AppResult<String> {
    ...
    if !crate::path_utils::is_safe_filename(&original_name) {
        return Err(crate::error::AppError::InvalidPath("Unsafe filename".to_string()));
    }
