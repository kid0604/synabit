...
fn extract_article_card(
    ...
    let heading_link_sel =
        Selector::parse("h1 a[href], h2 a[href], h3 a[href], h4 a[href]").ok()?;
    let primary_link = container
    ...
    if BLACKLIST_PATHS
        .iter()
        .any(|bp| path_lower.contains(&bp.to_lowercase()))
    {
        return None;
    }
...
fn extract_summary(container: &ElementRef, title: &str) -> String {
    ...
    for cls in &["excerpt", "summary", "description", "desc", "intro"] {
        ...
        let text = Selector::parse(&sel_str)
            .ok()
            .and_then(|sel| {
                container
                    .select(&sel)
...
fn is_inside_excluded(element: &ElementRef) -> bool {
    ...
    nav_patterns
        .iter()
        .any(|p| classes.contains(p) || id.contains(p))
}
