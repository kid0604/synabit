...
pub fn decode_exact_payload<T: serde::de::DeserializeOwned>(decrypted: &[u8]) -> Option<T> {
    postcard::take_from_bytes(decrypted)
        .ok()
        .and_then(|(val, remainder)| if remainder.is_empty() { Some(val) } else { None })
}
...
pub fn validate_and_parse_remote_entry(
    ...
    match (entry_kind, &sync_payload) {
        ...
        (SyncEntryKind::Delete, SyncPayload::Delete) => {
            Err(InboxApplyFailureKind::UnsupportedDelete)
        }
        ...
        let parsed_payload = match validate_and_parse_remote_entry(
            ...
            Err(failure_kind) => {
                let (target_state, err_param) = match failure_kind {
                    InboxApplyFailureKind::Corrupt => (InboxState::Quarantined, Some(failure_kind.as_str())),
...
pub fn validate_push_ack_batch(
    ...
    for ack in accepted {
        if !dispatch_ops.contains(&ack.operation_id) {
            return Err(AppError::General("protocol error: accepted operation_id not in batch".into()));
        }
        if !seen_accepted.insert(ack.operation_id) {
            return Err(AppError::General("protocol error: duplicate operation_id in accepted".into()));
        }
    }
    ...
    for ack in rejected {
        if !dispatch_ops.contains(&ack.operation_id) {
            return Err(AppError::General("protocol error: rejected operation_id not in batch".into()));
        }
        if !seen_rejected.insert(ack.operation_id) {
            return Err(AppError::General("protocol error: duplicate operation_id in rejected".into()));
        }
        if seen_accepted.contains(&ack.operation_id) {
            return Err(AppError::General("protocol error: operation_id present in both accepted and rejected".into()));
        }
    }

    if seen_accepted.len() + seen_rejected.len() != dispatchable.len() {
        return Err(AppError::General("protocol error: missing operation_id outcome in push response".into()));
    }
...
pub async fn dispatch_durable_outbox_at(
    ...
    for record in &dispatchable {
        sync_ops.push(crate::db::sync_outbox::outbox_record_to_sync_operation(record)?);
    }
    ...
    match adapter.push(sync_ops).await {
        Ok(push_result) => {
            if let Err(val_err) = validate_push_ack_batch(&dispatchable, &push_result.accepted, &push_result.rejected) {
                let err_msg = match &val_err { AppError::General(m) => m.clone(), _ => val_err.to_string() };
                let mut db = match db_state.lock() { Ok(g) => g, Err(p) => p.into_inner() };
                if let Err(db_err) = db.schedule_outbox_batch_retry(vault_id, provider_id, &op_ids, &err_msg, now) {
            ...
            for record in &dispatchable {
                ...
                } else if let Some(rej_ack) = push_result.rejected.iter().find(|ack| ack.operation_id == record.operation_id) {
        ...
        Err(err) => {
            let err_msg = match &err { AppError::General(m) => m.clone(), _ => err.to_string() };
            let mut db = match db_state.lock() { Ok(g) => g, Err(p) => p.into_inner() };
            if let Err(db_err) = db.schedule_outbox_batch_retry(vault_id, provider_id, &op_ids, &err_msg, now) {
                return Err(AppError::General(format!("{err_msg}, DB error: {db_err}")));
            }
...
pub fn snapshot_c2b_runtime_raw(
    ...
    let inbox = stmt2.query_map(rusqlite::params![vault_id, provider_id], |row| {
        ...
        let remote_seq = match rseq_raw {
            Some(s) => Some(u64::try_from(s).map_err(|_| parse_err("malformed remote_seq in sync_inbox"))?),
            None => None,
        };
        let op_blob: Vec<u8> = row.get(5)?;
        let operation_id: [u8; 16] = op_blob.try_into().map_err(|_| parse_err("malformed operation_id in sync_inbox"))?;
        let doc_blob: Vec<u8> = row.get(6)?;
        let doc_hash: [u8; 32] = doc_blob.try_into().map_err(|_| parse_err("malformed doc_hash in sync_inbox"))?;
        let ph_blob: Option<Vec<u8>> = row.get(9)?;
        let payload_hash: Option<[u8; 32]> = match ph_blob {
            Some(b) => Some(b.try_into().map_err(|_| parse_err("malformed payload_hash in sync_inbox"))?),
            None => None,
        };
        Ok(InboxRow {
            vault_id: row.get(0)?,
    ...
    let inbox_pages = stmt3.query_map(rusqlite::params![vault_id, provider_id], |row| {
        ...
        let has_more = DbBridge::decode_inbox_page_bool(has_more_raw).map_err(|e| parse_err(&e.to_string()))?;
        let entry_count_raw: i64 = row.get(5)?;
        let entry_count = u32::try_from(entry_count_raw).map_err(|_| parse_err("malformed entry_count in sync_inbox_pages"))?;
        Ok(InboxPagesRow {
            vault_id: row.get(0)?,
            provider_id: row.get(1)?,
    ...
    let inbox_page_entries = stmt4.query_map(rusqlite::params![vault_id, provider_id], |row| {
        ...
        let page_ordinal = u32::try_from(ord_raw).map_err(|_| parse_err("malformed page_ordinal in sync_inbox_page_entries"))?;
        let op_blob: Vec<u8> = row.get(4)?;
        let operation_id: [u8; 16] = op_blob.try_into().map_err(|_| parse_err("malformed operation_id in sync_inbox_page_entries"))?;
    ...
    let outbox = stmt5.query_map(rusqlite::params![vault_id, provider_id], |row| {
        ...
        let operation_id: [u8; 16] = op_blob.try_into().map_err(|_| parse_err("malformed operation_id in sync_outbox"))?;
        let doc_blob: Option<Vec<u8>> = row.get(6)?;
        let doc_hash: Option<[u8; 32]> = match doc_blob {
            Some(b) => Some(b.try_into().map_err(|_| parse_err("malformed doc_hash in sync_outbox"))?),
            None => None,
        };
        let src_blob: Option<Vec<u8>> = row.get(7)?;
        let source_hash: Option<[u8; 32]> = match src_blob {
            Some(b) => Some(b.try_into().map_err(|_| parse_err("malformed source_hash in sync_outbox"))?),
            None => None,
        };
        let ph_blob: Option<Vec<u8>> = row.get(10)?;
        let payload_hash: Option<[u8; 32]> = match ph_blob {
            Some(b) => Some(b.try_into().map_err(|_| parse_err("malformed payload_hash in sync_outbox"))?),
            None => None,
        };
        let retry_raw: i64 = row.get(13)?;
        let retry_count = u32::try_from(retry_raw).map_err(|_| parse_err("malformed retry_count in sync_outbox"))?;
        Ok(OutboxRow {
            vault_id: row.get(0)?,
            provider_id: row.get(1)?,
            operation_id,
...
#[cfg(test)]
#[path = "../../../.agents/oracles/c2b_oracle_v3.rs"]
mod c2b_oracle_v3;
"""

with open('src-tauri/src/sync/coordinator.rs', 'w') as f:
    f.write(code)
print("Updated final coordinator_v1 successfully")
