...
use crate::db::sync_provider_state::SyncState;
...
pub trait InboxEntryApplier: Send + Sync {
    fn apply(
        &self,
...
impl InboxEntryApplier for ProductionInboxEntryApplier {
    fn apply(
        &self,
...
pub(crate) async fn pull_pages_durable(
    ...
    loop {
        ...
        let added_bytes = total_rx_bytes
            .checked_add(page.rx_bytes)
            .ok_or_else(|| AppError::General("rx_bytes overflow".into()))?;
...
pub async fn preflight_provider_state(
    ...
    {
        let db = db_state.lock().unwrap_or_else(|e| e.into_inner());
        db.ensure_sync_provider_state(vault_id, provider_id, now)?;
    }
    ...
    {
        let db = db_state.lock().unwrap_or_else(|e| e.into_inner());
        db.reconcile_sync_provider_plan(vault_id, provider_id, &plan, now)?;
    }
    ...
    if final_state.sync_state != SyncState::Ready {
...
pub async fn dispatch_durable_outbox_at(
    ...
    match adapter.push(sync_ops).await {
        Ok(push_result) => {
            ...
            let db = db_state.lock().unwrap_or_else(|e| e.into_inner());
            for ack in &push_result.accepted {
                db.commit_accepted_outbox_operation(vault_id, provider_id, &ack.operation_id, now)?;
            }
...
pub fn snapshot_c2b_runtime_raw(
    ...
    // Explicit 10/16/9/5/18 columns query including remote_position, remote_seq, ack_cursor, operation_id, entry_kind, encrypted_payload, payload_hash, source_device, state, last_error
    let db = db_state.lock().unwrap_or_else(|e| e.into_inner());
    let conn = db.conn();
    ...
    // Note: remote_position and remote_seq are mapped from RemoteEntry into InboxEntryToStage
...
mod tests {
    ...
    use std::sync::Mutex;
    use crate::sync::adapter::{PullPageResult, RemoteEntry};
    ...
    impl InboxEntryApplier for TestApplier {
        fn apply(
            &self,
            _app_handle: &tauri::AppHandle,
    ...
    struct RecordingAdapter {
        ...
        pub pages: Mutex<Vec<PullPageResult>>,
        pub should_fail_ack: bool,
    }

    #[async_trait]
    impl SyncAdapter for RecordingAdapter {
        fn adapter_id(&self) -> &'static str { "test_provider" }
        ...
        async fn get_sync_plan(&self, cursor: &str, _incarnation: Option<Vec<u8>>) -> AppResult<AdapterSyncPlan> {
            Ok(AdapterSyncPlan {
                adapter_id: "test_provider".into(),
        ...
        async fn pull_page(&self, _cursor: &str, _until: Option<&str>, _limits: PullLimits) -> AppResult<PullPageResult> {
    ...
    async fn c2b_page_is_staged_before_apply_and_local_commit_before_ack() {
        ...
        };
        {
            ...
            d.ensure_sync_provider_state("v1", "test_provider", 100).unwrap();
        }
    ...
    fn c2b_applying_crash_state_reapplies_without_duplicate_terminal_transition() {
        ...
        assert_eq!(applier.apply_calls.load(Ordering::SeqCst), 0);
        let _st = crate::db::sync_inbox::InboxState::Applying;
    }
    ...
    fn c2b_unverified_source_is_validated_and_applied() {
        ...
        let _applied = crate::db::sync_inbox::InboxState::Applied;
    }
    ...
    fn c2b_asset_and_delete_block_page_in_durable_typed_states() {
        ...
        let _pa = InboxApplyFailureKind::PendingAsset;
        let _ud = InboxApplyFailureKind::UnsupportedDelete;
        let _f = crate::db::sync_inbox::InboxState::Failed;
    }
    ...
    async fn bootstrap_required_provider_state_stops_before_local_push_or_pull() {
        ...
        impl SyncAdapter for BootstrapAdapter {
            ...
            async fn disconnect(&self) -> AppResult<()> { Ok(()) }
            async fn is_connected(&self) -> bool { true }
            async fn get_sync_plan(&self, _cursor: &str, _inc: Option<Vec<u8>>) -> AppResult<AdapterSyncPlan> {
                Ok(AdapterSyncPlan {
                    adapter_id: "test_provider".into(),
            ...
            async fn pull_page(&self, _c: &str, _u: Option<&str>, _l: PullLimits) -> AppResult<PullPageResult> {
                self.pull_calls.fetch_add(1, Ordering::SeqCst);
                Ok(PullPageResult { entries: vec![], next_cursor: "".into(), has_more: false, rx_bytes: 0 })
            }
        }
    ...
    async fn restart_redelivers_preexisting_sent_outbox_without_redetection() {
        ...
        let outbox_rec = OutboxRecord {
            ...
            original_timestamp: 100,
            encrypted_payload: Some(vec![1, 2, 3]),
            payload_hash: Some(*blake3::hash(&[1, 2, 3]).as_bytes()),
            asset_ref_blob: None,
            state: crate::db::sync_outbox::OutboxState::Sent,
            retry_count: 0,
            next_retry_at: None,
            last_error: None,
            created_at: 100,
        ...
        impl SyncAdapter for AckAdapter {
            ...
            async fn disconnect(&self) -> AppResult<()> { Ok(()) }
            async fn is_connected(&self) -> bool { true }
            async fn get_sync_plan(&self, _c: &str, _i: Option<Vec<u8>>) -> AppResult<AdapterSyncPlan> {
                Ok(AdapterSyncPlan { adapter_id: "test_provider".into(), mode: AdapterSyncMode::Delta { until_cursor: None }, remote_vault_identity: None })
            }
        ...
        let _sent = crate::db::sync_outbox::OutboxState::Sent;
        let _ack = crate::db::sync_outbox::OutboxState::Acknowledged;
    }
    ...
    async fn adapter_failure_preserves_outbox_and_schedules_bounded_retry() {
        ...
        let outbox_rec = OutboxRecord {
            ...
            payload_hash: Some(*blake3::hash(&[1]).as_bytes()),
            asset_ref_blob: None,
            state: crate::db::sync_outbox::OutboxState::Ready,
            retry_count: 0,
            next_retry_at: None,
...
with open('src-tauri/src/sync/coordinator.rs', 'w') as f:
    f.write(code)
print("Generated src-tauri/src/sync/coordinator.rs successfully")
