...
impl SyncCoordinator {
    ...
    pub async fn sync(
        ...
        let push_ops = {
            let db = db_state.lock().unwrap_or_else(|e| e.into_inner());
            db.get_dispatchable_outbox(&vault_id, &provider_id, chrono::Utc::now().timestamp_millis(), 100)?
        };

        let sync_ops: Vec<crate::sync::core::types::SyncOperation> = push_ops.into_iter().map(|outbox_record| {
            crate::sync::core::types::SyncOperation {
                operation_id: outbox_record.operation_id,
                doc_hash: outbox_record.doc_hash.unwrap_or_default(),
                entry_kind: outbox_record.entry_kind,
                node_id: outbox_record.node_id,
                rel_path: outbox_record.rel_path.unwrap_or_default(),
                encrypted_payload: outbox_record.encrypted_payload.unwrap_or_default(),
        ...
        // 4. Push
        let push_result = adapter.push(sync_ops).await?;
        ...
        let (plan, cursor) = {
            ...
            let plan = adapter.get_sync_plan(&cursor, None).await?;
            (plan, cursor)
        };
...
pub fn snapshot_c2b_runtime_raw(
    ...
    for (table, query) in queries {
        let mut stmt = conn.prepare(query).map_err(|e: rusqlite::Error| crate::error::AppError::General(e.to_string()))?;
        let cols: Vec<String> = stmt.column_names().into_iter().map(|s| s.to_string()).collect();
        let mut rows = stmt.query(rusqlite::params![vault_id, provider_id]).map_err(|e: rusqlite::Error| crate::error::AppError::General(e.to_string()))?;
        while let Some(row) = rows.next().map_err(|e: rusqlite::Error| crate::error::AppError::General(e.to_string()))? {
            let mut map = std::collections::HashMap::new();
            map.insert("table".to_string(), rusqlite::types::Value::Text(table.to_string()));
            for (i, col) in cols.iter().enumerate() {
                let val: rusqlite::types::Value = row.get::<_, rusqlite::types::Value>(i).map_err(|e: rusqlite::Error| crate::error::AppError::General(e.to_string()))?;
